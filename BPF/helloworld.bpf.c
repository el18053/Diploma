...
int trace_ondemand_readahead_enter(struct pt_regs *ctx) {
	...
	if (v != NULL) {
		...
		if (*v == uid) {
			...
			bpf_printk("ondemand_readahead started with req_size=%d", req_size);
		}
	}
	return 0;
...
int trace_page_cache_next_miss_exit(struct pt_regs *ctx) {
	...
	if (v != NULL) {
		...
		if (*v == uid) {
			int ret = 0;
			ret = PT_REGS_RC(ctx);
			bpf_printk("page_cache_next_miss exited with ret=%d", ret);
		}
	}
	return 0;
...
int trace_do_page_cache_ra(struct pt_regs *ctx) {
	...
	if (v != NULL) {
		...
		uid = bpf_get_current_pid_tgid();
		if (*v == uid) {
			u32 req_size = PT_REGS_PARM2(ctx);
...
int trace_page_cache_ra_unbounded(struct pt_regs *ctx) {
	...
	if (v != NULL) {
		...
		if (*v == uid) {
			...
			u32 lookahead_size = PT_REGS_PARM3(ctx);
